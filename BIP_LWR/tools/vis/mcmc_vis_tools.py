...
def list_paths(base_path=""):
    ...
    for root, dirs, files in os.walk(base_path+'MCMC_outputs'):
        ...
        if len(files)>0 and files[0][-2:]=='h5':
            print(os.path.join(*(root.split(os.path.sep)[1:])))
            path_list.append(root)
    return path_list

def list_S3_dirs(dir_name=None, year='2019', region='USEast'):
    """
    List all directories with MCMC chains on S3. If nothing is passed
    to 'dir_name', then the current directory name will be used

    Parameters
    ----------
    dir_name: str or None
        Name of directory to filter by (ex: "April3_2018-tune_real_data")
        Default is None. This results in using the current directory
    year: str
        Either '2018' or '2019'
    region: str
        Either 'USEast' or 'London'. Default is USEast
    """
    if dir_name is None:
        cwd = os.getcwd()
    ...
    if region == 'USEast':
        bucket_name = 'lwr-inverse-us-east'
    elif region == 'London':
        bucket_name = 'lwr-inverse-mcmc'
    else:
        raise ValueError("'region' should be either 'USEast' or 'London'")
    lwr_inv = s3.Bucket(bucket_name)
    path_list = list(set([os.path.dirname(obj.key) for obj in lwr_inv.objects.filter(Prefix=os.path.join(year, dir_name))]))
    for elem in path_list:
        print(elem)
    return path_list

def download_chains_from_S3(data_directory, region='USEast'):
    """
    Download all chains from S3 into a local directory
    ...
    new_data_directory : str
        New path to use. It is simply data_directory with 'S3' after 'MCMC_outputs'
    """
    s3 = boto3.resource("s3")
    if region == 'USEast':
        bucket_name = 'lwr-inverse-us-east'
        ...
        raise ValueError("'region' should be either 'USEast' or 'London'")
    lwr_inv = s3.Bucket(bucket_name)
    for elem in lwr_inv.objects.filter(Prefix=data_directory):
        s3_path = elem.key
        # remove directory 'flow_poisson-70108.. (etc)' if it exists
        ...
        if s3_path[-2:] != 'h5':
            continue
        # download chain
        util.download_chain(s3_path=s3_path, local_path=local_path, bucket_name=bucket_name)
    new_data_directory = os.path.join(*local_path.split("/")[:-1])
